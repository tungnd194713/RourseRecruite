        ...
        <div class="mt-0 mt-lg-2 pt-0 pt-lg-3 pb-2">
          <div class="row">
            ...
            <div class="col-12 col-lg-6 mt-4 mt-lg-0">
              ...
              <div class="row mb-4">
                <div class="col-12">
                  <button id="btn-apply" class="btn btn-secondary w-100 h4">
                    応募する
                  </button>
                </div>
              </div>
...
export default {
  ...
  computed: {
    previewImageJobUrl() {
      return this.job.image_job
        ? process.env.API_URL.replace('api', 'storage') + this.job.image_job
        : ''
    },
    ...
    previewDateEnd() {
      return this.$moment(this.job.date_start)
        .add(this.job.display_month, 'M')
        .format('YYYY/MM/DD')
    },
  },
  ...
  methods: {
    ...
    previewFormRecruitment() {
      return this.job.form_recruitment
        ? this.formRecruitmentList.filter(this.filterPreviewFormRecruitment)[0]
            .text
        : null
    },
    ...
    async getJobFromApi() {
      const { data } = await this.$repositories.jobs.getJob(
